...
(() => {
  ...
  if(loginForm) loginForm.onsubmit=async e=>{
    ...
    try{
      const r=await fetch(SUPABASE_URL+'/auth/v1/token?grant_type=password',{method:'POST',headers:{'apikey':KEY,'Content-Type':'application/json'},body:JSON.stringify({email:document.getElementById('email').value.trim(),password:document.getElementById('password').value})});
  ...
  if(signup) signup.onclick=async()=>{
    ...
    try{
      const r=await fetch(SUPABASE_URL+'/auth/v1/signup',{method:'POST',headers:{'apikey':KEY,'Content-Type':'application/json'},body:JSON.stringify({email,password})});
      const text=await r.text();let d;try{d=text?JSON.parse(text):null}catch{d=text}
