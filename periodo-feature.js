...
function renderPeriodoReport(scroll){
 ...
 table.querySelector('thead tr').innerHTML='<th>Data</th><th>Equipe</th><th>Período</th><th>Setor</th><th>Viagens</th><th>Volume (m³)</th><th>Material</th><th class="detail-head">Detalhes</th>';
 document.getElementById('reportTable').innerHTML=rs.map(r=>`<tr><td>${r.data}</td><td>${esc(eq(r.equipe_id)?.nome||'-')}</td><td>${labelPeriodo(r.periodo||'integral')}</td><td>${esc(st(r.setor_id)?.nome||'-')}</td><td>${r.viagens}</td><td>${fmt(r.volume)}</td><td>${esc(r.material||'-')}</td><td class="detail-cell"><button class="actionbtn" onclick="window.openRegistroDetalhes&&window.openRegistroDetalhes('${r.id}')">👁 Ver detalhes</button></td></tr>`).join('')||'<tr><td colspan="8">Nenhum resultado para os filtros selecionados.</td></tr>';
